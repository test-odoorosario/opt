...
class Presentation:
    ...
    def get_numeroPartner(self, invoice):
        ...
        """
        return invoice.partner_id.vat if invoice.partner_id.property_account_position_id not in [
            ...
            ] else invoice.partner_id.country_id.vat
    ...
    def get_codigoOperacion(self, invoice):
        ...
        if not_taxed_taxes and (len(invoice.tax_line_ids) - len(other_taxes)) == len(not_taxed_taxes):
            res = 'N'

        if exempt_taxes and (len(invoice.tax_line_ids) - len(other_taxes)) == len(exempt_taxes):
            res = 'E'
    ...
    def get_invoice_other_taxes(self, invoice):
        ...
        return [
            tax for tax in invoice.tax_line_ids
            if tax.tax_id.tax_group_id in tax_group_ids
        ]
