...
class TestVatDiary(common.TransactionCase):
    ...
    def _create_partners(self):
        partner_proxy = self.env['res.partner'].with_context(mail_create_nosubscribe=True)
        vals = {
            'customer': True,
            'supplier': True,
            'name': 'Partner',
        ...
        self.partner_ri = partner_proxy.create(vals)
        vals.update({
            'property_account_position_id': self.env.ref('l10n_ar_afip_tables.account_fiscal_position_cf').id,
            'vat': '30000000002'
        })
        self.partner_cf = partner_proxy.create(vals)
